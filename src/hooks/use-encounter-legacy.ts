...
export function useEncounterLegacy(appointmentId: string) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const recordPromise = apptData.medicalRecordId
        ? medicalService
            .getMedicalRecordById(apptData.medicalRecordId)
            .catch((err: any) => {
  ...
  useEffect(() => {
    ...
    if (apptData.nextAppointmentDate) {
      setNextAppointmentDate(
        format(new Date(apptData.nextAppointmentDate), "yyyy-MM-dd"),
      );
    }
    ...
    if (recordData) {
      setMedicalRecord((prev) => {
        ...
        const normalized: MedicalRecord = {
          ...recordData,
          id: recordData.id,
          currentMedications: normalizeStringArray(
          ...
          alcoholConsumption: normalizeBoolean(
            (recordData as any).alcoholConsumption,
          ),
    ...
    if ((recordData as any)?.latestVitalSign) {
      ...
      latest = {
        ...
        createdAt: lv.recordedAt
          ? new Date(lv.recordedAt).toISOString()
  ...
  // Logic kiểm soát cửa sổ mở lại hồ sơ (48h)
  const isWithinReopenWindow = (() => {
    const record = encounterData?.recordData;
    ...
    return diffHours <= 48; // Constant 48h per BE policy
  })();
  ...
  const buildUpdateDto =
    useCallback((): UpdateMedicalRecordDtoForEncounter | null => {
  ...
  const updateRecord = useCallback(
    (field: keyof MedicalRecord, value: unknown) => {
  ...
  const updateLinking = useCallback((record: LinkRecordInfo | null) => {
    ...
    if (record) {
      toast.success(
        "Đã chọn hồ sơ: " + (record.recordNumber || "Hồ sơ gần nhất"),
      );
    }
  }, []);
  ...
  const saveMutation = useMutation({
    mutationFn: async (variables: {
      dto: UpdateMedicalRecordDtoForEncounter;
      snapshot: string;
    }) => {
      return await appointmentService.updateMedicalRecord(
        appointmentId,
        variables.dto,
      );
    },
    onSuccess: (_data, variables) => {
      // Update last saved snapshot to the one we actually sent
      lastSavedSnapshotRef.current = variables.snapshot;
      setAutoSaveTime(new Date());
      setAutoSaveStatus("saved");

      // Only reset dirty flag if nothing has changed since we started this specific save
      if (getCurrentFormSnapshot() === variables.snapshot) {
        setIsDirty(false);
      }
  ...
  const saveDraft = useCallback(async () => {
    ...
    try {
      ...
      if (!dto) return;
      await saveMutation.mutateAsync({ dto, snapshot });
    } catch (error) {
      throw error;
    }
  }, [medicalRecord, buildUpdateDto, getCurrentFormSnapshot, saveMutation]);
  ...
  const performAutoSave = useCallback(async () => {
    ...
    saveMutation.mutate({ dto, snapshot: currentSnapshot });
  }, [
    canEdit,
    medicalRecord,
    isDirty,
  ...
  const completeMutation = useMutation({
    mutationFn: async (payload: { id: string; dto: any }) => {
      return await appointmentService.completeExamination(
        payload.id,
        payload.dto,
      );
    },
